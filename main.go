...
func main() {
	...
	app.Get("/api/pegawai", func(c *fiber.Ctx) error {
		...
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Failed To Query Database" + err.Error()})
		}
	...
	app.Post("/api/pegawai", func(c *fiber.Ctx) error {
		...
		stmt, err := db.Prepare(`
			INSERT INTO pegawai
			(nip, nama_lengkap, jabatan, jenis_kelamin, golongan, pangkat, unit_kerja, skpd, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id
		`)
		...
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"message": "Failed to Insert Database" + err.Error(),
			})
		}
	...
	app.Put("/api/pegawai/:id", func(c *fiber.Ctx) error {
		...
		if err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
				"message": "Failed to Parse Code" + err.Error(),
			})
		}
	...
	app.Delete("/api/pegawai/:id", func(c *fiber.Ctx) error {
		...
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"message": "Failed to Execute" + err.Error(),
			})
		}
